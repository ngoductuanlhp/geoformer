...
class FSInstDataset:
    ...
    def load_scene_graph_info(self):
        ...
        load_path = os.path.join(self.data_root, self.dataset, 'scene_graph_info_train.pkl')
        ...
        load_path = os.path.join(self.data_root, self.dataset, 'scene_graph_info_test.pkl')
        with open(load_path, 'rb') as handle:
    ...
    def get_test_comb(self):
        test_combs_file = os.path.join(self.data_root, self.dataset, 'test_combinations_fold' + str(cfg.cvfold) + '.pkl')
    ...
    def get_support_set(self, k_shot):
        ...
        support_set_file = os.path.join(self.data_root, self.dataset, 'support_sets', cfg.type_support + str(cfg.cvfold) + '_' + str(k_shot) + 'shot_10sets.pkl')
    ...
    def get_sphere_inst(self, support_xyz_middle, support_instance_label, support_instance_id, scale_factor = 2):
        ...
        offset_centroid = np.abs(inst_pc - centroid)
    ...
    def get_region_inst(self, support_xyz_middle, support_instance_label, support_instance_id, scale_factor = 2):
        ...
        valid_pc =  (support_xyz_middle[:, 0] <= x_upper) & (support_xyz_middle[:, 0] >= x_lower) &\
                    (support_xyz_middle[:, 1] <= y_upper) & (support_xyz_middle[:, 1] >= y_lower) &\
                    (support_xyz_middle[:, 2] <= z_upper) & (support_xyz_middle[:, 2] >= z_lower)
    ...
    def trainMergeFS(self, ids):
        ...
        for idx, id in enumerate(ids):
            ...
            while True:
                ...
                if query_scene_name+'.npy' in self.geodist_train_files:
                    break

            with open(os.path.join(self.data_root, self.dataset, 'geo_dist_train', '%s.npy' % query_scene_name), 'rb') as handle:
            ...
            scene_info = self.scene_graph_info_train[query_scene_name]
            max_len = scene_info['locs_float_'].shape[0]
            ...
            support_locs.append(torch.cat([torch.LongTensor(support_xyz_scaled.shape[0], 1).fill_(idx), \
                torch.from_numpy(support_xyz_scaled).long()], 1))
            ...
            scene_infos.append({'sampled_class': sampled_class, 'query_scene': query_scene_name, 'support_scene': support_scene_name, 'support_instance_id': support_instance_id})
        ...
        ### voxelize
        support_voxel_locs, support_p2v_map, support_v2p_map = pointgroup_ops.voxelization_idx(support_locs, self.batch_size, self.mode)
        
        support_pc_mins = torch.stack(support_pc_mins).float()
        support_pc_maxs = torch.stack(support_pc_maxs).float()

        support_dict = {'voxel_locs': support_voxel_locs, 'p2v_map': support_p2v_map, 'v2p_map': support_v2p_map,
                        'locs': support_locs, 'locs_float': support_locs_float, 'feats': support_feats,
                        'support_masks': support_masks,'spatial_shape': support_spatial_shape,
                        'batch_offsets': support_batch_offsets,
                        'pc_mins': support_pc_mins, 'pc_maxs': support_pc_maxs}
        ...
        query_dict = {'voxel_locs': query_voxel_locs, 'p2v_map': query_p2v_map, 'v2p_map': query_v2p_map,
                    ...
                    'labels': query_labels, 'instance_labels': query_instance_labels, 
    ...
    def testMergeFS(self, ids):
        ...
        with open(os.path.join(self.data_root, self.dataset, 'geo_dist_test', '%s.npy' %query_scene_name), 'rb') as handle:
            geo_dist_pkl = pickle.load(handle)

        geo_dist = []
        scene_info = self.scene_graph_info_test[query_scene_name]
        max_len = scene_info['locs_float_'].shape[0]
        for i, k in enumerate(geo_dist_pkl.keys()):
            geo = np.ones((max_len)) * 10
            geo[geo_dist_pkl[k]['vertex'].astype(np.long)] = geo_dist_pkl[k]['dis']
        ...
        if cfg.fix_support:
            ...
            for l in test_comb['active_label']:
                ...
                support_dict = {'voxel_locs': support_voxel_locs, 'p2v_map': support_p2v_map, 'v2p_map': support_v2p_map,
                                'locs': support_locs, 'locs_float': support_locs_float, 'feats': support_feats,
                                'support_masks': support_masks,'spatial_shape': support_spatial_shape,
                                'batch_offsets': support_batch_offsets, 'mask_offsets': support_masks_offset, 
                                'pc_mins': support_pc_min, 'pc_maxs': support_pc_max}
