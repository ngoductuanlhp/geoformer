...
from criterion_dyco import InstCriterion
from model.geoformer.dyco3d import DyCo3d
...
def train_one_epoch(
    ...
    for iteration, batch_input in enumerate(train_loader):
        ...
        if epoch > cfg.prepare_epochs and outputs['mask_logits'] is None:
            continue
        ...
        if iteration % 10 == 0:
            if epoch <= cfg.prepare_epochs:
                logger.info("Epoch: {}/{}, iter: {}/{} | lr: {:.6f} | loss: {:.4f}({:.4f}) | Sem loss: {:.4f}({:.4f}) | Off loss: {:.4f}({:.4f}) | Mem: {:.2f} | iter_t: {:.2f} | remain_t: {remain_time}\n".format
                    (epoch, cfg.epochs, iteration + 1, num_iter, scheduler.get_lr()[0], am_dict['loss'].val, am_dict['loss'].avg,\
                ...
                logger.info("Epoch: {}/{}, iter: {}/{} | lr: {:.6f} | loss: {:.4f}({:.4f}) | Sem loss: {:.4f}({:.4f}) | Off loss: {:.4f}({:.4f}) | Score loss: {:.4f}({:.4f}) | Dice loss: {:.4f}({:.4f}) | Mem: {:.2f} | iter_t: {:.2f} | remain_t: {remain_time}\n".format
                    (epoch, cfg.epochs, iteration + 1, num_iter, scheduler.get_lr()[0], am_dict['loss'].val, am_dict['loss'].avg,\
                    am_dict['sem_loss'].val, am_dict['sem_loss'].avg,\
                    am_dict['offset_norm_loss'].val, am_dict['offset_norm_loss'].avg,\
...
def main():
    ...
    if cfg.resume:
        ...
        if os.path.isfile(checkpoint_fn):
            ...
            start_iter = start_epoch * num_iter
            ...
            scheduler = PolyLR(optimizer, max_iter=max_iter, power=0.9, last_step=start_iter)
        else:
